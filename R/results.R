...
#d$ns <- factor(d$ns, levels=c(10,50,250), labels=paste('n',c(10,50,250),sep='=='))
#d$s <- factor(d$s, levels=c(0.01,0.1,1,10,100), labels=paste('sigma',c(0.01,0.1,1,10,100),sep='=='))
qplot(data=d ,x=s2, y=mean,color=factor(s1),xlab='True SD', ylab='Posterior Mean') + 
  facet_grid(facets=r~prior,scales='free') + geom_abline(1) + theme(legend.position= 'bottom') + scale_x_log10() + scale_y_log10()
